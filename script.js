...
const EXTRA = [
  ...
  { name:'Короб', unit:'п.м.', price:1200, step:1 },
...
function renderTable(selector, data){
  ...
  tbody.innerHTML = data.map(row => `
    <tr data-editable-price="${row.editablePrice ? '1':'0'}">
      <td>${row.name}${row.note?`<div class="kicker">${row.note}</div>`:''}</td>
...
function buildEstimate(){
  ...
  wrap.innerHTML = `<div style="overflow:auto;"><table class="table"><thead><tr><th>Позиция</th><th>Кол-во</th><th>Цена ед.</th><th>Сумма</th></tr></thead><tbody>
    ${items}
