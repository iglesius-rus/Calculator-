...
const MAIN = [
  ...
  { name:'Установка помпы', unit:'шт.', price:0, step:1, editablePrice:true }, // ставь цену в поле
...
const EXTRA = [
  ...
  { name:'Дозаправка фреоном', unit:'г.', price:7, step:100 }, // 7 руб/грамм, шаг 100г
...
function applyDiscountToTotal(total){
  const pct = getDiscountPct(); const discount = Math.round(total*pct)/100; const final = Math.max(0, total - discount);
  const line=document.getElementById('discount-line'); if (line){ if (pct>0){ line.style.display=''; document.getElementById('discount-pct').textContent=pct; document.getElementById('discount-amount').textContent=format(discount); document.getElementById('grand-with-discount').textContent=format(final);} else { line.style.display='none'; } }
...
function strictRecalc(){
  ...
  document.querySelectorAll('#table-main tbody tr, #table-extra tbody tr').forEach(tr => {
    ...
    const sumCell = tr.querySelector('.sum'); if (sumCell){ sumCell.textContent = format(sum)+' ₽'; sumCell.dataset.sum = sum; }
    subTotal += sum||0;
  });
  const grand = document.getElementById('grand-total'); if (grand) grand.textContent = format(subTotal);
  applyDiscountToTotal(subTotal);
  return subTotal;
}
...
function buildEstimate(){
  ...
  const d = applyDiscountToTotal(subTotal);
  const wrap = document.getElementById('estimate-body'); if (!wrap) return;
  if (!rows.length && d.pct<=0){ wrap.innerHTML = '<p class="kicker">Пока ничего не выбрано. Укажите количество и нажмите «Рассчёт».</p>'; return; }
  ...
  wrap.innerHTML = `<div style="overflow:auto;"><table class="calc-table"><thead><tr><th>Позиция</th><th>Кол-во</th><th>Цена ед.</th><th>Сумма</th></tr></thead><tbody>
    ...
    ${`<tr><td colspan="3" style="text-align:right;"><b>Итого со скидкой</b></td><td style="text-align:right;"><b>${format(d.final)} ₽</b></td></tr>`}
  </tbody></table></div>`;
