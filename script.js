...
function _demoActiveCount(exceptInput){
  let n = 0;
  document.querySelectorAll('td[data-label="Кол-во"] input[type="number"]').forEach(inp => {
    if (exceptInput && inp === exceptInput) return;
    if (_parseMoney(inp.value) > 0) n++;
  });
  return n;
}
function _demoUpdateBadge(){
  const badge = document.getElementById('demo-badge');
  if (!badge) return;
  if (!DEMO_MODE){ badge.style.display = 'none'; return; }
  const count = _demoActiveCount(null);
  badge.style.display = '';
  badge.textContent = `Демо: выбрано ${count}/${DEMO_MAX_ITEMS} позиций`;
}
function _demoToast(msg){
  const badge = document.getElementById('demo-badge');
  if (!badge) { try{ alert(msg); }catch(e){}; return; }
  badge.textContent = msg;
  badge.style.fontWeight = '700';
  badge.style.opacity = '1';
  setTimeout(()=>{ badge.style.fontWeight = ''; badge.style.opacity=''; _demoUpdateBadge(); }, 1400);
}
function _demoEnforceLimit(changedInput){
  if (!DEMO_MODE) return true;
  const v = _parseMoney(changedInput.value);
  if (v <= 0) return true;
  const others = _demoActiveCount(changedInput);
  if (others >= DEMO_MAX_ITEMS){
    changedInput.value = '';
    _demoToast(`Демо-версия: доступно только ${DEMO_MAX_ITEMS} позиции`);
    return false;
  }
  return true;
}
...
function attachEstimateUI() {
  ...
  if (btnPdf) {
    ...
    // qty inputs (демо-лимит по позициям)
  document.querySelectorAll('td[data-label="Кол-во"] input[type="number"]').forEach(inp => {
    const handler = () => { if (_demoEnforceLimit(inp)) recalcAll(); };
